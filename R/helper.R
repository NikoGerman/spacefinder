...
#' @description
#' Transforms task data to unit cube coordinates using the learner's fitted subspace,
#' filters points within the unit hypercube, and fits univariate beta distributions
#' to each hyperparameter dimension using weighted maximum likelihood estimation.
#'
#' @param object Subspace learner object (\code{LearnerSubspaceBox} or
#'   \code{LearnerSubspacePolygon}) with fitted subspace parameters
#' @param regularize `logical` whether to regularize fitted parameters to avoid
#'   U-shaped densities by enforcing \code{alpha >= 1} and \code{beta >= 1}.
#'   Default: \code{TRUE}
#' @param invert_fn `function` to invert transformation matrix A.
#'   Default: \code{\\(A) chol2inv(chol(A))} for general positive definite matrices.
#'   Box learner uses \code{\\(A) diag(1 / diag(A))} for diagonal matrices.
...
#'   \item{parameter}{hyperparameter name}
#'   \item{alpha}{fitted beta shape parameter}
#'   \item{beta}{fitted beta shape parameter}
#'   \item{converged}{whether MLE converged}
#'   \item{iterations}{number of Newton-Raphson iterations}
#'   \item{cat_hp}{categorical level (only if task has categorical hyperparameters)}
#'
#' @details
#' For each categorical level (or globally if no categorical hyperparameters):
#' \enumerate{
#'   \item Transforms data: \eqn{x = A^{-1}(y - b)} to map subspace to unit cube
#'   \item Filters points where all coordinates lie in \eqn{[0,1]^d}
#'   \item Fits \eqn{Beta(\alpha, \beta)} to each dimension via weighted MLE
#'   \item Returns \eqn{Beta(1,1)} uniform prior if no valid points remain
#' }
#'
#' The transformation maps the fitted hyperrectangle (defined by matrix A and
#' translation vector b) to the unit hypercube, allowing density estimation in
#' a normalized space. Points falling outside the unit cube after transformation
#' are excluded from density fitting.
#'
#' @seealso
#' \code{\link{fit_beta_mle_single}} for the univariate beta MLE algorithm.
#' \code{\link{augment.LearnerSubspaceBox}} for the Box learner method.
#' \code{\link{augment.LearnerSubspacePolygon}} for the Polygon learner method.
augment_subspace_learner <- function(
  object,
  regularize = TRUE,
  invert_fn = \(A) chol2inv(chol(A)),
  ...
) {
  ...
  fit_level <- function(level_data, hps, A, b, w) {
    ...
    if (nrow(DT) == 0) {
      warning("No valid points in unit cube, returning uniform prior Beta(1,1)")
      return(data.table(
        parameter = hps,
        alpha = 1.0,
        beta = 1.0,
    ...
    # Fit beta MLE
    rbindlist(lapply(hps, function(hp) {
  ...
  # Process levels
  if (!is.null(cat_hps)) {
    rbindlist(lapply(unique(coefs$cat_hp), function(lvl) {
      coef_row <- coefs[cat_hp == lvl]
      fit_level(
        data[get(cat_hps) == lvl],
        coef_row$hyperparameters[[1]],
        coef_row$A[[1]],
        coef_row$b[[1]],
        data[get(cat_hps) == lvl, get(object$task$target_measure)]
      )[, cat_hp := lvl]
    }))
  } else {
    fit_level(
      data,
      coefs$hyperparameters[[1]],
      coefs$A[[1]],
      coefs$b[[1]],
      data[[object$task$target_measure]]
    )
  }
}
